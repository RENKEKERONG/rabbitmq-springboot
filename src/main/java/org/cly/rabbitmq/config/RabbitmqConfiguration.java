...
public class RabbitmqConfiguration {
    ...
    public final static String myFanoutExchange = "renke.fanout";
    public final static String myFanoutQueue = "renke.fanout.queue";
...
//    BindingBuilder.bind(queueEmailMessage()).to(topicExchange()).with("sb.*.email");
//    queueEmailMessage()是获取队列
//    当消费者监听queueEmailMessage该queue时，是要是生产者以sb.*.email发送过来的消息都会被接受
    @Bean
    ...
    //=============下面是我自己创建的pub&sub交换机乐行为fanout的模式
    ...
    //将queue绑定到交换机类型为fanout


    //    绑定
    @Bean
    public Binding myFanoutBindQueue(){
        return BindingBuilder.bind(myFanoutQueue()).to(myFanoutExchange());
    }
    //===========我自己创建的pub&sub交换机乐行为fanout的模式结束
...
//    @Bean
//    public FanoutExchange myFanoutExchange01(){
